...
const APY_API_URL = 'https://api.compound.finance/api/v2/ctoken?meta=true&network=mainnet'
...
const getCompoundApy = async (strategy: String) => {
  try {
    ...
    const apyData = compoundInfo.cToken.filter(
      (it: any) => it.token_address && String(it.token_address).toLowerCase() === String(strategy).toLowerCase(),
    )
